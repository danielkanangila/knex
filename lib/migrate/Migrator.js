...
const InteractiveMake = require('./InteractiveMake');
...
class Migrator {
  ...
  // Creates a new migration, with agiven data from interactive prompt
  interactiveMake(name, config) {
    this.config = getMergedConfig(config, this.config);
    new InteractiveMake(name, this.generator, config);
  }
