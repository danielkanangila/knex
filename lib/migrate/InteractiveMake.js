...
class InteractiveMake {
  ...
  completed$() {
    ...
    const columnsStr = this._generateColumns(this.columnSchema);
    const generatedFileName = this._generator(this.tableName, columnsStr);
    return this.resolve(generatedFileName);
  }
  ...
  async _generator(tableName, columns) {
    ...
    return await this.generator.make(tableName, config);
  }
